...
public class DiscordLogPlayerEmbedMessage implements DFPluginMessageHandler {
    @Override
    public void run(PluginMessageEvent event, ByteArrayDataInput in) {
        if (event.getSource() instanceof ServerConnection serverConnection) {
            ...
            DFProxyPlugin.discord.thread.onPluginDiscordPlayerEmbedMessage(plr, server, colour, fullString);
        }
    }
}
